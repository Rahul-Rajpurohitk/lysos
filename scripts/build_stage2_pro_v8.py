"""Build stage2-pro-v8 — pro-v5 + ALL teacher distillation (5 layers).

Layers:
  1. Chem teacher (5,000)              — Designer<->Critic loops with PDB anchors
...
Total teacher distillation: 43,500 traces

Output: data/processed/amr-stage2-pro-v8
"""
...
OUT_DIR = ROOT / "data" / "processed" / "amr-stage2-pro-v8"
...
def main():
    ...
    for label, path in TEACHER_FILES:
        ...
        print(f"  {label:35s} +train {n_added_train:>5,}  +valid {n_added_valid:>4,}")
    ...
    print(f"\n✅ stage2-pro-v8: train={len(train_ds):,} valid={len(valid_ds):,} test={test_n}")
