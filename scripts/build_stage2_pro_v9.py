"""Build stage2-pro-v9 — pro-v5 + ALL teacher distillation across 6 layers.
...
Layers (no API spend; all manual inline):
  ...
  6. Targeted teacher (17,150)         per-PDB pockets + per-mutation + 3-way + self-correct + indications + tool chains

Total teacher distillation: 60,650 traces

Output: data/processed/amr-stage2-pro-v9
"""
...
OUT_DIR = ROOT / "data" / "processed" / "amr-stage2-pro-v9"
...
def main():
    ...
    for label, path in TEACHER_FILES:
        ...
        print(f"  {label:32s} +train {n_added_train:>6,}  +valid {n_added_valid:>5,}")
    ...
    print(f"\n✅ stage2-pro-v9: train={len(train_ds):,} valid={len(valid_ds):,} test={test_n}")
