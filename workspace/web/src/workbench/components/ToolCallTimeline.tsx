  ...
  return (
    <div className="border-b border-slate-200 last:border-0">
      <button
        ...
        className="w-full flex items-start gap-1.5 px-2.5 py-1.5 text-left hover:bg-slate-50"
      >
        <Icon className="w-3 h-3 mt-1 text-slate-400 shrink-0" />
        <StatusIcon className={`w-3 h-3 mt-1 shrink-0 ${statusColor}`} />
        <div className="flex-1 min-w-0">
          <div className="font-mono text-[11.5px] text-emerald-700 font-semibold break-all leading-tight">
            {call.tool}
          </div>
          <div className="flex items-center gap-1.5 text-[10px] text-slate-500 font-mono mt-0.5 flex-wrap">
            <span className="px-1 rounded bg-slate-100 text-slate-600">{call.agent}</span>
            <span className="text-slate-300">·</span>
            <span>{call.duration_ms}ms</span>
            <span className="text-slate-300">·</span>
            <Clock className="w-2.5 h-2.5" /> <span>{tsLabel(call.created_at)}</span>
          </div>
        </div>
      </button>
      {expanded && (
